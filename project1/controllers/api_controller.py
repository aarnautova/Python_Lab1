...
class GetById(Resource):
    ...
    def get(self, id):
        """ Method for HTTP-get request """
        return storage.read(id).__dict__()
...
class Add(Resource):
    ...
    def post(self):
        ...
        result = storage.create(watering_period.ob_from_dict(period)).__dict__()
...
class Update(Resource):
    ...
    def put(self):
        ...
        if period.get("period_id") is None:
            return "", 404
        storage.update(period.get("period_id"), days=period.get("days"), time=period.get("time"))
        storage.save_session()
        return 200


class Delete(Resource):
    """ Class for deleteng some watering period """
    def delete(self, id):
